...
remoteUpgrade_t getUpgradeFileInfo(remoteUpgrade_t fileInfo,char *recvData)
{
  ...
  { 
   ...
   {
    if((cJSON_GetObjectItem(jasonObj,"downurl")==NULL)||
	 (cJSON_GetObjectItem(jasonObj,"md5")==NULL)||
	 (cJSON_GetObjectItem(jasonObj,"version")==NULL)||
	 (cJSON_GetObjectItem(jasonObj,"content")==NULL))
    {
      fileInfo.firmwareCheckStatus=FIRMWARE_ERROR_NETWORK_UNREACHABLE;
	  fileInfo.checkVersionStatus=0;
 	...
 	strncpy(fileInfo.downloadUrl,cJSON_GetObjectItem(jasonObj,"downurl")->valuestring,sizeof(fileInfo.downloadUrl));
...
firmreCheck_t getUpgradeFile(const char *url,const char *md5) {
	...
	memset(cmdBuf, 0, sizeof(cmdBuf));
	sprintf(cmdBuf,"cd /tmp ; wget -t 2 -T 20 -O fw.bin %s",url);
    system("rm -rf /tmp/wgetok");
    ...
    {
        system(cmdBuf);
        system("echo 1 > /tmp/wgetok");
        exit(EXIT_SUCCESS);
    }

    if(pid < 0)
    {
        ret =FIRMWARE_ERROR_DOWNLOAD_FAIL;
        return ret; 
    }
    ...
    if (find_pid_by_name("wget") > 0)
	{
        DPrintf("find and kill wget.\n");
