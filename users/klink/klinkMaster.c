...
KlinkNode_t* setMeshLinklistDataToMib_1(KlinkNode_t*head)
{	
		...
		{  
		   ...
		   case SLAVE1:
			...
			if(strncmp(tmpBuf,tmpBuf2,sizeof(tmpBuf))==0)
			 break;
			if(strncmp(tmpBuf,tmpBuf2,12)&&(strlen(tmpBuf2)!=0))
				goto slave2;
			memset(tmpBuf2,0,sizeof(tmpBuf2));
	        apmib_set(MIB_KLINK_SLAVE1_SOFT_VERSION, (void *)tmpBuf);
			apmib_set(MIB_KLINK_SLAVE1_SN, (void *)tmpSn);
...
/*search target slave node*/
 KlinkNode_t* serchTargetSlaveNode(KlinkNode_t*head,KlinkNode_t *pdata)
{
	KlinkNode_t *phead=head;
	if(phead==NULL)
	{
...
KlinkNode_t * getSlaveNode(cJSON *messageBody)
{
  KlinkNode_t *pTargetSlaveNode=NULL;
  KlinkNode_t data;
  memset(&data,0x00,sizeof(KlinkNode_t));
  strcpy(data.slaveDevideInfo.slaveMacAddr,cJSON_GetObjectItem(messageBody,"sourceMac")->valuestring);
  pTargetSlaveNode=serchTargetSlaveNode(g_pKlinkHead,&data);
}
...
const char* masterGenerateJsonMessageBody(int messageType,cJSON *messageBody,char** pMessage)
{
 ...
 switch(messageType)
 {
  case KLINK_SLAVE_REPORT_DEVICE_INFO:                     	//messageType=0
  	{
  	 KlinkNode_t *pSlaveNode=getSlaveNode(messageBody);
	 if(pSlaveNode==NULL)
	 {
	   cJSON_AddItemToObject(topRoot, "repAck", root = cJSON_CreateObject());
	   cJSON_AddStringToObject(root, "ack", "none");
	 }
	 else
	 {
	  cJSON_AddItemToObject(topRoot, "repAck", root = cJSON_CreateObject());
	  cJSON_AddStringToObject(root, "ack", pSlaveNode->slaveDevideInfo.slaveFwVersion);	  
	 } 
  	}
	...
	if(messageType>=1)
	  {
	    ...
	    {
            TRACE_DEBUG("Error! destMac not match, destMac = %s\n", cJSON_GetObjectItem(pJson,"destMac")->valuestring);
            cJSON_Delete(pJson);
		    return 0;
	    }
	  }
...
int main(int argc , char *argv[])
{
	...
	//g_pKlinkHead=initKlinkListHead();
	 g_lastMasterCheckTime = upSecond();
         if(g_pKlinkHead!=NULL)
         destroyKlinkList(g_pKlinkHead);
	 g_pKlinkHead=initKlinkListHead(); 
